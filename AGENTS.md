...
- Prettier is the formatting gate. There is no ESLint configuration or test suite yet; do not claim
  coverage that does not exist.
- `npm run verify` is the repository gate: formatting, the ECS dependency guard, and the TypeScript
  build must all pass.
